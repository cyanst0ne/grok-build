...
pub async fn get_installer() -> Option<&'static str> {
    ...
    // Ignore shared ~/.grok/config.toml installer: official grok may have
    // persisted "internal", which would pull https://x.ai/cli over this fork.
    Some("gh-release")
}

/// The npm entry links to a binary inside the package, so the running
/// executable's real path names the installer.
#[allow(dead_code)] // retained for tests; get_installer always returns gh-release
